  ...
  return (
    <div style={{ minHeight: "calc(100vh - 200px)", display: "grid", gridTemplateColumns: "1fr 1fr" }}>
      ...
      <div style={{ background: "var(--green-900)", color: "var(--cream-100)", padding: 64, position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
        <div style={{ position: "absolute", inset: 0, background: "repeating-linear-gradient(135deg, rgba(244,234,213,0.04) 0 14px, transparent 14px 28px)" }}/>
        ...
        <div style={{ position: "relative", zIndex: 2 }}>
          ...
          <p style={{ marginTop: 24, fontSize: 16, color: "rgba(244,234,213,0.8)", maxWidth: 400, lineHeight: 1.6 }}>
            {a.sideBody}
          </p>
        </div>
        <div style={{ position: "relative", zIndex: 2, display: "flex", flexDirection: "column", gap: 12, fontSize: 13 }}>
          {a.sideBenefits.map((t,i) => <div key={i} style={{ color: "rgba(244,234,213,0.85)" }}>{t}</div>)}
        </div>
          ...
          {mode === "login" ? (
            <>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 24 }}>
                ...
                <label className="checkbox"><input type="checkbox" defaultChecked/><span>Continuar conectado nesta máquina</span></label>
                <button className="btn btn-primary btn-lg btn-block" onClick={()=>go("conta")}>Entrar →</button>
              </div>
            ...
            <>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 24 }}>
                <div className="field"><label className="field-label">Nome completo</label><input className="input" placeholder="João da Silva"/></div>
                ...
                <div className="field"><label className="field-label">Cria uma senha</label><input className="input" type="password" placeholder="Mínimo 8 caracteres"/></div>
                <div className="field"><label className="field-label">Sua propriedade</label><input className="input" placeholder="Sítio Boa Vista — Pouso Alegre/MG"/></div>
                <label className="checkbox"><input type="checkbox" defaultChecked/><span>Aceito os termos de uso e a política de privacidade</span></label>
                <button className="btn btn-primary btn-lg btn-block">Criar conta →</button>
              </div>
            </>
          )}
        </div>
